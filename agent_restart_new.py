...
def fetch_and_verify_github_file(api_host, owner, repo, file_path_in_repo, branch, local_filepath, github_token):
    """
    ...
    print(f"{datetime.datetime.utcnow().isoformat()} Attempting to fetch {full_api_url} using GitHub API (Token Auth)...")
    logger.info(f"Attempting to fetch {full_api_url} using GitHub API (Token Auth)...")
    ...
    try:
        ...
        if e.response.status_code == 401:
            print("Check your GITHUB_TOKEN and its permissions.")
            logger.error("Check GITHUB_TOKEN and its permissions.")
        return False
...
def retry_fetch_thread(initial_host_list, systemtype, domain):
    ...
    for attempt in range(1, NUM_OF_RETRIES + 1):
        ...
        fetch_success = fetch_and_verify_github_file(
            ...
            LOCAL_EXCLUSION_FILEPATH, github_token
        )
...
def main(systemtype, domain):
    ...
    if not github_token:
        print(f"{datetime.datetime.utcnow().isoformat()} Error: GITHUB_TOKEN environment variable not set. Exiting.")
